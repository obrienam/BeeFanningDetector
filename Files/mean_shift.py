...
width, length = (800, 800)         #2d array that holds flags cooresponding to whether or not a bee has both been in that position
beeframes = [[0] * width] * length #and whether it is pointed towards the hive.
for i in range(len(beeframes)):
    for j in range(len(beeframes[i])):
        beeframes[i][j] = 0
